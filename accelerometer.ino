...
void loop()
{
    ...
    lcd.print(" Roll = "); lcd.println(roll);
    lcd.print(" Yaw = "); lcd.println(yaw);
...
void getAngle(int Ax,int Ay,int Az) 
{
    ...
    roll = atan(x/sqrt((y*y) + (z*z))); //pitch calculation
    pitch = atan(y/sqrt((x*x) + (z*z))); //roll calculation
    yaw = atan(z/sqrt((x*x) + (y*y)));  // yaw calculation

    //converting radians into degrees
    pitch = pitch * (180.0/3.14) + 180;
    roll = roll * (180.0/3.14)  + 180;
    yaw = yaw * (180.0/3.14) + 180;
}
